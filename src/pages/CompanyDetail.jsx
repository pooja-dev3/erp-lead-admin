...
const CompanyDetail = () => {
  ...
  const handleDelete = async () => {
    if (confirm(`⚠️ WARNING: Are you sure you want to permanently delete ${company?.name}? This action cannot be undone and will remove all associated data including visitors, leads, and event history.`)) {
      setIsSubmitting(true);
      ...
      try {
        ...
        if (error.response?.status === 403) {
          showError('Access denied. Only Platform Admins can delete companies.');
        } else if (error.response?.status === 404) {
          const errorData = error.response?.data;
          if (errorData?.error === 'Route not found') {
            showError('Delete functionality not available. The DELETE endpoint has not been implemented on the server.');
          } else {
            showError('Delete endpoint not available. Please contact administrator.');
          }
        } else {
      ...
      <div className="md:flex md:items-center md:justify-between">
        ...
        <div className="mt-4 flex md:ml-4 md:mt-0 space-x-3">
          ...
          <button
            ...
            className={`btn ${String(company?.status).toLowerCase().trim() === 'active' || String(company?.status).toLowerCase().trim() === 'true' ? 'btn-secondary text-red-600 hover:text-red-700' : 'btn-secondary text-green-600 hover:text-green-700'} disabled:opacity-50 disabled:cursor-not-allowed`}
          >
          ...
          {isPlatformAdmin && (
            <button
              ...
              disabled={isSubmitting}
              className="btn btn-secondary text-red-600 hover:text-red-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
